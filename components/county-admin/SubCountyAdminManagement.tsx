...
interface SubCountyAdminManagementProps {
    users: User[];
...
const SubCountyAdminManagement: React.FC<SubCountyAdminManagementProps> = ({ users, onAddUser, onUpdateUser, onRemoveUser, currentUser }) => {
    ...
    const subCountyAdmins = useMemo(() => 
        users.filter(user => user.role === UserRole.SUB_COUNTY_ADMIN), 
        [users]
    );
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Sub-County Admin Management</h2>
            ...
            <p className="mb-6 text-gray-500 dark:text-gray-400">
                Manage Sub-County Admins within {currentUser.assignedCounty} county.
            </p>
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-700 dark:text-gray-300">
                 ...
                 {subCountyAdmins.length === 0 && <p className="text-center py-8 text-gray-500">No sub-county administrators found.</p>}
            </div>
            ...
            {isAddModalOpen && (
                <AddUserModal 
                    ...
                    defaultRole={UserRole.SUB_COUNTY_ADMIN}
                    allowedRoles={[UserRole.SUB_COUNTY_ADMIN]}
                    predefinedData={{ 
                        assignedRegion: currentUser.assignedRegion, 
                        assignedCounty: currentUser.assignedCounty 
                    }}
                    lockedFields={['assignedRegion', 'assignedCounty']}
                />
            )}
            {isBulkModalOpen && (
                <BulkRegisterModal
...
export default SubCountyAdminManagement;