...
import { PART_C_CRITERIA } from '../constants';
...
const CriterionRow: React.FC<{
  ...
  const getDisplayIndex = (idx: number) => {
    if (idx < 8) return idx + 1; // For criteria 1-8
    if (idx >= 8 && idx <= 10) return 9; // For 9a, 9b, 9c
    return idx - 1; // For criteria 10-13 (indices 11-14)
  };

  return (
    <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
      <td className="py-3 px-4 text-center">{getDisplayIndex(index)}</td>
...
const ScoreSheetC: React.FC<ScoreSheetProps> = ({ scores, onScoreChange }) => {
  return (
    <div>
      <h3 className="text-xl font-bold mb-4">Part C: Scientific Thought, Method & Creativity (35 Marks)</h3>
       <div className="overflow-x-auto bg-white dark:bg-gray-800 shadow-md rounded-lg">
        <table className="w-full text-sm text-left text-gray-700 dark:text-gray-300">
          ...
          <tbody>
            {PART_C_CRITERIA.map((criterion, index) => (
              <CriterionRow 
                key={criterion.id}
                criterion={criterion}
